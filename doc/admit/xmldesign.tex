...
the pipeline has created for them. They will also be able to re-run
...
that the archive produces Project$\rightarrow$Source$\rightarrow$Band Cube
$\rightarrow$Line Cube.
...
an image cube.
\item
Line List: a list of all lines detected in a band, but there is (can?) also a bandmerged version
\item
...
\subsection{notes}
Note that there can be a Description Vector per Line Cube, but also per source, or even per project.
For example, a Description Vector in a Line Cube could be Feature based (e.g. the moments of inertia)
...
Should VP's retain the P-S-B/L structure?
...
version. Linux command: {\tt cp -al}, Mac command: {\tt pax -rwl}. But
rsync is more portable, but harder to use. 

Let assume the user has downloaded a few admit files, for projects 
p1 and p2 in this case.
...
p1/admit.xml              (via admit)
   p1.s1.b1.fits          (from archive if also downloaded)
   p1.s1.b2.fits
   p1.s1.b3.fits
   p1.s1.b4.fits
   p1.s2.b1.fits
   ...
   p1.s2.b3.fits
   p1.s2.b4.fits
   s1/l1/l1_cube.im       (a recomputed cube)
