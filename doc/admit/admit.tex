...
these will be combined into one single source mosaiced image datacube.
...
Each plugin will expect information from previous steps to be present.
...
but aided by cross-correllation techniques in a PV diagram, or the cube itself,
aided by where the sources are (see next two steps).
...
(clumpfind, dendrograms, gaussclumps, see list below in Section XX), 
...
polarization here?} the previously described steps produce ancillary data for the
{\it admit.zip} file.
...
but the JPG and/or FITS maps) into the {\tt admit.zip} file.
...
relevant code for the source and feature extraction problem.
...
timeline and the deliverables. All in a self-contained package.
