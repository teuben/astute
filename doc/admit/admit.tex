...
Based on the line identification a line cube can be derived,
with now the third axis in doppler velocity space. This also eases
comparison between the different lines and molecules.
...
possibly bringing the data volume down. 
In addition, there is an option to re-bin the cubes.
...
step). Moment maps are exported to {\tt admit.zip}.
...
are allowed to be hierarchical in nature
...
Description vectors based on moment 0,1,2.  For a selected object , one can
then ask what other objects have similar description vectors.
...
recompute (with perhaps modified parameters) the products created
in the archive.
...
A typical ZIP archive file might contains a descriptive XML file and 
...
projects or sources, and perform the same operations on each of
those. This can be done either in parallel, or recursively. It can then
extract information from each project and in essence mine a large
suite of data, use linked data techniques to visualize the extracted
