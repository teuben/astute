...
type model, where you do redo the statistics based on the knowledge where
...
this summary is also a simple table with properties such as 
...
but aided by cross-correllation techniques in PV diagrams or the cube itself,
aided by where the source material is (see next two steps).
The output will be a series of identified and unidentified lines in the different
bands (and sources), NL in total.
...
More sophisticated source extraction routines are available from the literature
(clumpfind, dendrograms, gaussclumps, etc.etc), which essentially produce
a table of properties for each clump. This table is stored as a VO Table 
in {\tt admit.zip}

\item
{\bf Source Characterization}
Since for each of the NL line's, a catalog of sources/clumps is avaibale,
characterization of the emission accross different molecules is now 
possible on a grand scale never been possible before. Principle Component
Analysis (PCA) and an Overlap Integral (OI) are two ways to quantify
and visualize this. A linked data tool (e.g. {\tt glue}) can be of
help here.
...
the projects, {\bf S} for the sources, {\bf B} for the bands (spectral windows), and 
{\bf L} for the identified and unidentified lines. They count to 
NP projects, NS sources, NB bands and NL lines.
...
\subsection{Pipeline: run a single project}
...
\subsection{User: simple project archive pull}
...
\subsection{User: code2}

In this case the user has decided to select another undefined line, add this 
to the ADMIT database, in addition, suggest a feedback to the ALMA data archive
(speculative). Recomputing and re-correlating data is an essential ingredient
of ADMIT.
...
other means), and datamining these dataset with ADMIT tools.
...
is written in casapy, and would be worthwhile studying.
